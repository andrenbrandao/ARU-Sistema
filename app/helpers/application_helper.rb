module ApplicationHelper
	...
	def sortable(column, title = nil, approved = true)
		title ||= column.titleize
		css_class = (column == sort_column) ? "current #{sort_direction}" : nil
		direction = (column == sort_column && sort_direction == "asc") ? "desc" : "asc"
		link_to title, {:sort => column, :direction => direction, approved: approved}, {:class => "#{css_class}", :remote => true }
	end
	...
	def display_all_errors resource
		...
		messages = resource.errors.map { |att, msg| content_tag(:p, msg) }.join
		html = <<-HTML
		<div class="alert alert-error alert-block">
			<button type="button" class="close" data-dismiss="alert">&#215;</button>
			#{messages}
